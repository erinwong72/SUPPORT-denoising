function []=Run_xtalk_PCA_ICA_RMmov_FanLab_function_automask(dt)
...
for t0 = 1:nframes
    ...
    % Calculate mean intensity in ring pixels for this frame
    ringSignal(t0) = mean(frame(ringMask));
    
    % Subtract ring mean from the entire frame
...
xlabel('Frame');
ylabel('Mean Ring Intensity');
title('Ring Signal Over Time');

% Save both plots together
saveas(gcf, 'RingMaskAndSignal.fig');
