function [] = simple_spike_T(IntensOrig, save_dir, blueStim, varargin)
...
%
% Optional name/value (batch use after visual QC):
%   'SpikeThresh', [] (default) — interactive getpts on Tconv; or scalar (all cells) or length-nCells vector

ip = inputParser;
addParameter(ip, 'SpikeThresh', [], @(x) isnumeric(x) && (isempty(x) || isvector(x)));
parse(ip, varargin{:});
spikeThreshUser = ip.Results.SpikeThresh(:).'; % row vector
...
if ~isempty(spikeThreshUser) && ~isscalar(spikeThreshUser) && numel(spikeThreshUser) ~= nCells
    error('simple_spike_T: SpikeThresh must be [], a scalar, or a vector with %d elements (nCells).', nCells);
end
if isscalar(spikeThreshUser)
    spikeThreshUser = repmat(spikeThreshUser, 1, nCells);
end
use_fixed_thresh = ~isempty(spikeThreshUser);
clear C;
for i = 1:nCells
...
% Write the file using this for thresholding
