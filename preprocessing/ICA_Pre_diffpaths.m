...
% Same as ICA_Pre, but the directories to retrieve and save data are
% different
%
% Inputs:
...
function [nCell, t, icsTime_all, icsTimeOrig_all, icsSpace_all, CellImgs, MaskMov] = ICA_Pre_diffpaths(session_path,denoised_path, save_dir,is_stim, use_ring_bkg, use_support)
...
if use_support
   ...
   % save_dir = '/Volumes/fanlab/Labmembers/Niveda/Testing/Noisy_Test/2';
    ...
    dirs = dir(denoised_path);
    for i = 1:length(dirs)
        if contains(dirs(i).name, "_denoised") && ~isdir(dirs(i).name)
            mov = loadtiff(fullfile(denoised_path, dirs(i).name));
        end
    end
    
    imshow(mov(:,:,10), [])
    % mov_raw = loadtiff(fullfile(save_dir, "SertChRmine23_121109_Spon30AO4_1.tif"));
   % imshow(mov_raw(:,:,10), [])
    sprintf("loaded mov file: %s", fullfile(denoised_path, "denoised.tif"))
else
